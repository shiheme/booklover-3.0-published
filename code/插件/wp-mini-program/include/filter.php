...
// 小鱼哥 增加钩子：修改默认restapi关键字路径
add_filter('rest_url_prefix', function () {
	return wp_miniprogram_option('newrestapi');
});
...
add_filter( 'comment_type_list', function( $post_id, $type ) {
	...
	foreach ( $comments as $comment ) {
		...
		// 小鱼哥 增加字段 评论时间
		$_data["comment_date"] = $comment->comment_date;
...
add_filter( 'rest_posts', function( $posts, $request ) {
	...
	foreach ( $posts as $post ) {
		...
		if ($post_type == 'library') {
			...
			$_data["state"] = $state;
			$_data["book_pagestyle"] = get_post_meta($post->ID, 'book_pagestyle');
			$_data["book_foreground"] = get_post_meta($post->ID, 'book_foreground');}
		//小鱼哥
		if ($post_type == 'app'||$post_type == 'films'||$post_type == 'pro'||$post_type == 'movie') {
		...
		if ($post_type == 'topic') {
			...
			if ($book_tolibrary[0] != false) {
				...
				foreach ($book_tolibrary as $key => $app_k) {
					if ($key >= 5) break;
					$__book_tolibrary[] = array(
						"id" => pods_field_display('library', $app_k['ID'], 'ID'),
						"book_thumb" => wp_get_attachment_image_src(get_post_thumbnail_id($app_k['ID']), 'medium')[0],
						"book_pagestyle" => pods_field_display('library', $app_k['ID'], 'book_pagestyle'),
						"book_foreground" => pods_field_display('library', $app_k['ID'], 'book_foreground'),
					);}
		...
		if ($post_type == 'quot') {
			...
			if ($book_tolibrary[0] != false) {
				...
				foreach ($book_tolibrary as $key => $app_k) {
					...
					$__book_tolibrary[] = array(
						"id" => pods_field_display('library', $app_k['ID'], 'ID'),
						"book_author" => pods_field_display('library', $app_k['ID'], 'book_author'),
						"book_title" => pods_field_display('library', $app_k['ID'], 'book_title'),
						"book_thumb" => wp_get_attachment_image_src(get_post_thumbnail_id($app_k['ID']), 'medium')[0],
						"book_pagestyle" => pods_field_display('library', $app_k['ID'], 'book_pagestyle'),
						"book_foreground" => pods_field_display('library', $app_k['ID'], 'book_foreground'),
					);}
		...
		if ($post_type == 'act') {
			$_data["act_shopads"] = get_post_meta($post->ID, 'act_shopads');
			$_data["act_shopname"] = get_post_meta($post->ID, 'act_shopname');
			$_data["act_starttime"] = get_post_meta($post->ID, 'act_starttime');
			$_data["act_endtime"] = get_post_meta($post->ID, 'act_endtime');
			$_data["act_timelong"] = get_post_meta($post->ID, 'act_timelong');
		}
		// 小鱼哥 添加自定义字段结束
...
// 小鱼哥 增加钩子 喜欢/打卡的数据
add_filter('miniprogram_commented_msg', function ($post_id, $user_id, $type) {
	$user = get_user_by('ID', $user_id);
	if (!$user) {
		return false;
	}
	$args = array('post_id' => $post_id, 'type__in' => array($type), 'user_id' => $user_id, 'status' => 'approve');
	$comments = get_comments($args);
	$likemsg = array();
	foreach ($comments as $comment) {
		$_data = array();
		...
		$_data["comment_date"] = $comment_date;
		$likemsg[] = $_data;
	}
	return $likemsg;
}, 10, 3);
...
/**
 * 小鱼哥增加自定义设置的缓存（注意缓存后无法自动更新）
 */

add_filter( 'wp_rest_cache/allowed_endpoints', function ( $allowed_endpoints ) {
    if ( ! isset( $allowed_endpoints[ 'mp/v1' ] ) || ! in_array( 'setting', $allowed_endpoints[ 'mp/v1' ] ) ) {
