...
class WP_REST_Qrcode_Router extends WP_REST_Controller {
	...
	public function wp_post_qrcode( $request ) {
		$post_id = $request['id'];
		$post_cnttype = isset($request['cnttype']) ? $request['cnttype'] : get_post_type( $post_id );
		...
		if( $post_type != 'custom' ) {
			$post_path = '/pages/index/index?gotourl=/pages/detail/detail&id='.$post_id.'&isshare=1&posttype='.$post_type.'&cnttype='.$post_cnttype; //小鱼哥 修改了二维码路径
		} else {
