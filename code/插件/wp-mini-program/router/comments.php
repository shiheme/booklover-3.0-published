	...
	public function wp_post_comments( $request ) {
		...
		foreach ($comments as $comment) {
			...
			if($parent == 0) {
				...
				$_data['post']['post_title']= $post->post_title;
				$_data['post']['post_type']= $post->post_type;
				$_data['post']['post_status']= $post->post_status;
				$_data['post']['comment_status']= $post->comment_status;
				$_data['post_thumbnail'] = apply_filters( 'post_thumbnail', $post_id);
				// $_data["thumbnail"] = apply_filters( 'post_thumbnail', $post_id );
				...
				$_data["rating"] = get_comment_meta( $comment->comment_ID, 'rating', true );
	...
	public function insert_wp_posts_comment( $request ) {
		...
		if($type == 'comment') {
			...
			if($comment_id) {
				...
				$ratingflag = false;
				if($rating != '') {
					$ratingflag = add_comment_meta($comment_id, 'rating', $rating, true); 
				}
				$likes = (int)get_comment_meta( $comment_id, 'likes', true );
