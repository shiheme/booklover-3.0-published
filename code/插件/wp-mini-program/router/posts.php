...
class WP_REST_Posts_Router extends WP_REST_Controller {
	...
	public function get_most_posts( $request ) {
		$data = array();
		$page = isset($request["page"])?$request["page"]:1;
		$posttype = isset($request["post_type"])?$request["post_type"]:'posts';
		$per_page = isset($request["per_page"])?$request["per_page"]:10;
		$offset = ($page * $per_page) - $per_page;
		$meta = isset($request["meta"])?$request["meta"]:'views';
		$orderby = isset($request["orderby"])?$request["orderby"]:'meta_value_num';
		$args = array( 'posts_per_page' => $per_page, 'offset' => $offset, 'meta_key' => $meta, 'post_type' => $posttype,'orderby' => $orderby, 'order' => 'DESC', 'update_post_meta_cache' => false, 'cache_results' => false );
		$args = apply_filters( "rest_post_query", $args, $request );
		$query  = new WP_Query();
		$posts = $query->query( $args );
		if($posts) {
	...
	public function get_relate_posts( $request ) {
		...
		if( $post_id ) {
			...
			$args = array( 'posts_per_page' => $per_page, 'offset' => $offset, 'orderby' => 'rand', 'order' => 'DESC', 'post_type' => $posttype,'post__not_in' => array( $post_id ), 'tag__in' => $post_tag );
		}
		$query  = new WP_Query();
		$posts = $query->query( $args );
		if($posts) {
